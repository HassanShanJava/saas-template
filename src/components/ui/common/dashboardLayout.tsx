    ...
    <div className="font-poppins flex h-full w-full relative ">
      ...
      >
        ...
        >
              ...
              <>
                {!dropdown && (
                  <Link
                    key={i}
                    to={link}
                    className={`flex items-center gap-2 rounded-md p-1 transition-colors hover:bg-primary ${isSidebarOpen ? "justify-start text-sm" : "justify-center text-lg"} ${isActiveLink(link, hiddenRoutes) ? "bg-primary" : ""}`}
                  >
                    <div
                      className={`w-7 h-7 ${isActiveLink(link, hiddenRoutes) ? "bg-[#3ED13E]" : "bg-gray-100"} rounded-lg justify-center flex items-center`}
                    >
                      <Icon
                ...
                {dropdown && children && (
                  <Accordion type="single" collapsible>
                    <AccordionItem
                      key={i}
                      value="item-1 !border-none" id="accordion">
                      <AccordionTrigger
          ...
          {orgId !== 21 && sidePanel && sidePanel?.map((item: any, i: number) => (
            <>
