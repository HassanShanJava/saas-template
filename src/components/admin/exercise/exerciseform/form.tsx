...
const ExerciseForm: React.FC<ExerciseFormProps> = ({ isOpen, setOpen }) => {
  ...
  const FormSchema = z.object({
    ...
    category_id: z.coerce
      ...
      .default(0),
      equipment_ids: z.array(z.number()).nonempty({
        message: "Required",
      }),
    primary_muscle_ids:  z.array(z.number()).nonempty({
        message: "Required",
      }),
    secondary_muscle_ids: z.array(z.number()).optional(),
    primary_joint_ids:z.array(z.number()).nonempty({
        message: "Required",
      }),
    created_by: z
      .number({
  ...
  return (
    <Sheet open={isOpen}>
      <SheetContent hideCloseButton className="!max-w-[1100px]">
                                  ...
                                  {field.value
                                    ? CategoryData?.find(
                                        (category: baseExerciseApiResponse) =>
                                          category.value === field.value // Compare with numeric value
                                      )?.label // Display category name if selected
                                    : "Select Category*"}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="w-full ">
                          ...
                          <MultiSelect
                                floatingLabel="Select Equipments"
                                options={
                                    EquipmentData as {
                                        label: string;
                                        value: string | number;
                                ...
                                defaultValue={Array.isArray(field.value) ? field.value : []} // Ensure defaultValue is always an array
