...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(login.fulfilled, (state, { payload }) => {
      ...
      state.userInfo = {
        user: payload.user,
        sidepanel: payload.sidepanel,
        accessLevels: payload.accessLevels,
        pospanel: payload.pospanel,
      };
      ...
      localStorage.setItem(
        "accessLevels",
        JSON.stringify(payload.accessLevels)
      );
...
export const login = createAsyncThunk(
  "auth/login",
  async (
    { email, password, rememberme, website_url }: loginParams,
    { rejectWithValue }
  ) => {
    try {
      const {
      ...
      const { data: userResource } = await getUserResource(
        data.user.role_id,
      ...
      if (userResource && accessLevels) {
        ...
        const posPanel = filterUserResource.find(
          (item: ResourceTypes) => item.is_root && item.code == "pos"
        );
