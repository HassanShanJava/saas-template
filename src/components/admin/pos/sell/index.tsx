...
const Sell = () => {
  ...
  const isContinue =
    JSON.parse(localStorage.getItem("isContinue") as string) || null;
  ...
  const { data: memberhsipList } = useGetMembershipsQuery({
    org_id: orgId,
    query: query,
  });
  const { data: memberList, refetch: memberRefetch } = useGetAllMemberQuery({
    org_id: orgId,
    query: "sort_key=id&sort_order=desc",
  });
  ...
  const { data: transactionData, refetch: transactionRefetch } =
    useGetTransactionQuery(
      {
        counter_id: counter_number as number,
        query: "status=Unpaid&sort_order=desc",
      },
      {
  ...
  const updateFormValues = (updatedPayload: typeof productPayload) => {
    const subtotal = updatedPayload.reduce(
      (acc, prod) => acc + prod.sub_total,
      0
    );
    const totalTax = updatedPayload.reduce(
      (acc, prod) => acc + prod.tax_amount,
      0
    );
    const totalDiscount = updatedPayload.reduce(
      (acc, prod) => acc + prod.discount,
      0
    );
  ...
  const addProduct = (product: any, type: string) => {
    ...
    const discountAmount =
      Math.round(((product.net_price * discountPercentage) / 100) * 100) / 100;
    const discountedPrice =
      Math.round((product.net_price - discountAmount) * 100) / 100;
    ...
    if (orgTaxType?.tax_type === "inclusive") {
      ...
      taxAmount =
        Math.round(
          (discountedPrice - discountedPrice / (1 + taxRate / 100)) * 100
        ) / 100;
        ...
        updatedPayload = prevPayload.map((prod) =>
          prod.item_id === product.id
            ? {
                ...
                discount:
                  Math.round((prod.discount + discountAmount) * 100) / 100,
                total: Math.round((prod.total + total) * 100) / 100,
                tax_amount:
                  Math.round((prod.tax_amount + taxAmount) * 100) / 100,
              }
            : prod
        );
      } else {
        ...
        .map((product) => {
          if (product.item_id === productId) {
            ...
            const discountAmount = roundToTwoDecimals(
              (product.discount / product.quantity) * newQuantity
            );
            ...
            const taxAmount =
              product.tax_type === "inclusive"
                ? roundToTwoDecimals(
                    discountedPrice -
                      discountedPrice / (1 + product.tax_rate / 100)
                  )
                : roundToTwoDecimals(
                    discountedPrice * (product.tax_rate / 100)
                  );
  ...
  useEffect(() => {
    if (discountAmt) {
      ...
      const totalTax = productPayload.reduce(
        (acc, prod) => acc + prod.tax_amount,
        0
      );
  ...
  const validateCheckout = () => {
    if (productPayload.length == 0) {
      toast({
        variant: "destructive",
        title:
          "Please add at least one item to the receipt to enable checkout.",
      });
      return false;
    }
  ...
  useEffect(() => {
    if (retriveSaleData) {
      ...
      const membership_plans = items?.filter(
        (item) => item.item_type == "membership_plans"
      );
      ...
      setCustomer(
        memberListData?.find(
          (member) => member.id == payload.member_id
        ) as MemberTableDatatypes
      );
    ...
    <div>
      <FormProvider {...form}>
        <div className="p-5">
              ...
              <div className="grid grid-cols-1  slg:grid-cols-2 justify-start items-start gap-3">
                <div className="min-h-36  p-2">
                  <FloatingLabelInput
                    ...
                    icon={
                      <Search className="size-4 text-gray-400 absolute  z-10 left-2" />
                    }
                  />

                  <Tabs
                    defaultValue="membership_plans"
                        ...
                        {category.products.length > 0 ? (
                          <div className="mt-4 w-full flex flex-wrap gap-4 justify-center items-center">
                            {category.products.map(
                              (product: Record<string, any>, i) => (
                                <button
                                  key={i}
                                  disabled={
                                    Number(watcher.id) ||
                                    (id && watcher.transaction_type == "Refund")
                                      ? true
                                      : false
                                  }
                                  onClick={() =>
                                    addProduct(product, category?.type)
                                  }
                                ...
                                >
                                  ...
                                  {!watcher.id ||
                                    (id &&
                                      watcher.transaction_type == "Refund" && (
                                        <span className="absolute invisible group-hover:visible  bottom-0   text-black/80 text-sm z-20 p-1">
                                          Add
                                        </span>
                                      ))}
                                </button>
                          ...
                          <p className="col-span-2 text-sm text-center w-full p-2 mt-2">
                            {category.empty}
                          </p>
                        )}
                      </TabsContent>
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <Button
                        ...
                        className=" text-white justify-center items-center gap-2"
                      >
                        ...
                        <>
                          <div className=" flex  justify-between items-center gap-2  p-2 ">
                            <div>
                              ...
                              {product.discount > 0 && (
                                <p className="line-through text-sm text-gray-500 text-left">
                                  {formatToPKR(product.sub_total)}
                                </p>
                              )}
                              <p className="text-sm">
                                {formatToPKR(
                                  product.sub_total - product.discount
                                )}
                            ...
                            <div className="inline-flex items-center ">
                              ...
                              <div className=" border-t border-b border-gray-100 text-gray-600 hover:bg-gray-100 inline-flex items-center px-4 py-0 select-none">
                                {product.quantity}
                              </div>
                              <button
                                disabled={
                  ...
                  <div className="rounded-sm bg-white/90 mx-1">
                    <div className="space-y-2 px-2 bg-gray-100 pt-2 ">
                      <div className="w-full flex gap-2  items-center justify-between">
                        <p>Subtotal</p>
                        <p>{formatToPKR(watcher.subtotal)}</p>{" "}
                        {/* Display Subtotal */}
                      </div>
                      ...
                      <div className="w-full flex gap-2 items-center justify-between">
                        <p>Tax</p>
                        <p>{formatToPKR(watcher.tax_amt)}</p>{" "}
                        {/* Display Tax */}
                      </div>
                      ...
                      <div className="w-full flex gap-2 items-center justify-between font-bold">
                        ...
                        <p>
                          {formatToPKR(
                            (id && watcher.transaction_type == "Refund"
                              ? -Number(watcher.total)
                              : watcher.total) as number
                          )}
                        </p>{" "}
                        {/* Display Total */}
                      </div>
                                      ...
                                      staffList.map((staff: any) => (
                                        <CommandItem
                                          ...
                                          onSelect={() => {
                                            ...
                                            setValue("staff_name", staff.label);
                                          }}
                                        >
                                          <Check
                                            className={cn(
                      ...
                      <Button
                        className="w-full bg-primary text-black rounded-sm"
          ...
          <DayExceeded
            isOpen={dayExceeded}
            onClose={handleCloseDayExceeded}
            onContinue={handleContinueDayExceeded}
            closeModal={() => setDayExceeded(false)}
          />
        </div>

        <ParkReceipt
          isOpen={openParkSale}
          onClose={() => setParkSale(false)}
          parkSale={parkSale}
        />
      </FormProvider>
  ...
  return (
    <AlertDialog open={isOpen} onOpenChange={closeModal}>
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle>
            Register is Open more than 24 Hours
          </AlertDialogTitle>
          <AlertDialogDescription className="text-sm">
            Do you want to continue with this register session or close to
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogCancel
            className="bg-primary text-black border-transparent hover:"
            onClick={onContinue}
          >
            Continue
          </AlertDialogCancel>
...
export function CustomerCombobox({
  ...
  label,
  disable,
}: customerComboboxTypes) {
  const modifiedList = customerList?.map((item: any) => ({
    value: item.id,
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value
            ? modifiedList?.find((customer: any) => customer.value == value)
                ?.label
            : label}
                ...
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    ...
                    const customer = customerList?.find(
                      (item: MemberTableDatatypes) =>
                        item.first_name + " " + item.last_name == currentValue
                    );
...
export function RetriveSaleCombobox({
  list,
  setCustomer,
  customer,
  label,
            ...
            <CommandGroup className="">
              {modifiedList?.map((modCustomer) => (
                <CommandItem
                  ...
                  onSelect={(currentValue) => {
                    const customerId = modifiedList.find(
                      (item) => item.transactionId == currentValue
                    ).member_id;
                  ...
                  <div className="flex gap-3 text-xs  justify-between items-center w-full">
                    <div>
                      <div className="flex gap-1">
                        {modCustomer.items
                          .slice(0, 1)
                          .map((list: SellItem, index: number) => (
                            <div key={index}>
                      ...
                      <p className="text-xs">
                        Parked by {displayDate(modCustomer.transaction_date)},
                        by{" "}
                        <span className="capitalize">
                          {modCustomer.staff_name}
                        </span>
                      </p>
                    </div>
